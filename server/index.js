...
const rijksKey = process.env.RIJKS_APIKEY;
const parisMuseesKey = process.env.PARISMUSEES_APIKEY;

async function fetchMetArtworks(limit = 100) {
...
async function fetchRijksArtworks(limit = 100) {
  const res = await fetch(
    `${RIJKS_API}?key=${rijksKey}&imgonly=true&type=painting&ps=20`
  );
...
app.post("/api/search", async (req, res) => {
  try {
    // Appel aux APIs en parallèle
    const [vamResults, gettyResults, chicagoResults] = await Promise.all([
      fetchMetArtworks(),
      fetchRijksArtworks(),
      fetchChicagoInstituteArtworks(),
    ]);

    const allArtworks = vamResults
      .concat(gettyResults, chicagoResults)
